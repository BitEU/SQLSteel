...
if exist sqlsteel_univac.exe del /Q sqlsteel_univac.exe
if exist sqlsteel_univac.o del /Q sqlsteel_univac.o
if exist *.o del /Q *.o
echo.
...
echo Compiling sqlsteel.c...
gcc -c -DUNIVAC -O2 -Wall -Wextra -std=c99 sqlsteel.c -o sqlsteel.o
gcc -c -DUNIVAC -O2 -Wall -Wextra -std=c99 database.c -o database.o
gcc -c -DUNIVAC -O2 -Wall -Wextra -std=c99 sqlengine.c -o sqlengine.o
gcc -c -DUNIVAC -O2 -Wall -Wextra -std=c99 query.c -o query.o
gcc -c -DUNIVAC -O2 -Wall -Wextra -std=c99 display.c -o display.o
...
echo Linking...
gcc -o sqlsteel_univac.exe sqlsteel.o database.o sqlengine.o query.o display.o -lm
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo.
    echo To run the database, type: sqlsteel_univac.exe
    echo.
    goto :EOF
) else (
    echo.
...
REM Clean previous build artifacts
echo Cleaning previous build artifacts...
if exist sqlsteel_mingw.exe del /Q sqlsteel_mingw.exe
if exist sqlsteel.obj del /Q sqlsteel.obj
if exist *.o del /Q *.o
echo.
...
gcc %WARNING_FLAGS% %OPTIMIZE_FLAGS% %PERF_FLAGS% ^
    -o sqlsteel_mingw.exe ^
    sqlsteel.c database.c sqlengine.c query.c display.c ^
    %LINKER_FLAGS% -lm
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: sqlsteel_mingw.exe

    REM Display file size
    for %%A in (sqlsteel_mingw.exe) do (
        echo File size: %%~zA bytes
    )
    echo.
    echo Optimization level: MAXIMUM (-O3 + LTO + native CPU^)
    echo.
    echo To run the database, type: sqlsteel_mingw.exe
    echo.
) else (
    echo.
    echo ========================================
...
cl /W4 %MSVC_OPTIMIZE% /Fe:sqlsteel.exe sqlsteel.c database.c sqlengine.c query.c display.c /link %MSVC_LINKER%
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: sqlsteel.exe

    REM Display file size
    for %%A in (sqlsteel.exe) do (
        echo File size: %%~zA bytes
    )
    echo.
    echo Optimization level: MAXIMUM (MSVC /O2 + Whole Program Optimization^)
    echo.
    echo To run the database, type: sqlsteel.exe
    echo.
) else (
    echo.
    echo ========================================
...
REM   gcc -g -O0 -DDEBUG -Wall -Wextra sqlsteel.c -o sqlsteel_debug.exe -lm
REM
REM For profile-guided optimization with GCC:
REM   Step 1: gcc -O3 -fprofile-generate sqlsteel.c -o sqlsteel_pgo.exe -lm
REM   Step 2: Run sqlsteel_pgo.exe with typical usage patterns
REM   Step 3: gcc -O3 -fprofile-use sqlsteel.c -o sqlsteel_optimized.exe -lm
REM
REM For static analysis:
REM   gcc -Wall -Wextra -Wpedantic -Wformat=2 -Wconversion sqlsteel.c -lm
REM
REM ============================================================================
